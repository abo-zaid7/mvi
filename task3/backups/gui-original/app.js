...
const state = {
  ...
  loadedDataset: null,     // "brisc" or "busi" - which scans the model expects
...
/* The three groups the model list is split into, in the order they appear.
 * BRISC Task 1, BRISC Task 2, then the group's BUSI networks - which are Task 2
 * as well, but a different dataset, so they get a heading of their own rather
 * than being mixed in with ours. */
const MODEL_GROUPS = [
  { dataset: "brisc", task: 1, label: "models.task1" },
  { dataset: "brisc", task: 2, label: "models.task2" },
  { dataset: "busi", task: 2, label: "models.busi" },
];
...
function renderModels() {
  ...
  MODEL_GROUPS.forEach(({ dataset, task, label }) => {
    const group = state.models.filter(
      (model) => model.task === task && model.dataset === dataset);
    ...
    /* If the scans that go with these models are not on the machine, say so
     * here rather than letting somebody load one and find an empty picker. */
    const info = state.datasets[dataset];
    if (info && !info.present) {
      const warning = document.createElement("p");
      warning.className = "hint warn";
      warning.textContent = i18n.t("models.missingData", info.expected_at);
